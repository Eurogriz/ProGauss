...
def pbc_c(rho, sigma, tau):
    ...
    xt2 = sigma/rho**(8/3)
    xs02 = 2**(5/3)*xt2            # channel: (sigma/4)/(rho/2)^(8/3)
    t_kin = tau/rho**(5/3)         # empirical: unpol 5.1.1 kernel
    u = min(xt2/(8*t_kin), 1.0)
    xi2 = (xs02 - xt2)**2 * (3*np.pi**2)**(-2/3)/4
    C0 = C0_1/(1 + xi2)**4
    g_tot = f_pbe_zeta(rs, 0.0, xt2)
    br0 = max(f_pbe_zeta(rs*2**(1/3), 1.0, xs02), g_tot)*0.5
    br1 = max(f_pbe_zeta(rs*2**(1/3), -1.0, xs02), g_tot)*0.5
    f0 = -(1 + C0)*u**2*(br0 + br1) + (1 + C0*u**2)*g_tot
...
print("sigma=0: ref", ref(rho, 0.0, tau), " mine", pbc_c(rho, 0.0, tau),
      " diff", abs(ref(rho, 0.0, tau) - pbc_c(rho, 0.0, tau)))
rho, sigma, tau = 0.7, 0.3, 0.6
print("test pt: ref", ref(rho, sigma, tau), " mine", pbc_c(rho, sigma, tau),
      " diff", abs(ref(rho, sigma, tau) - pbc_c(rho, sigma, tau)))
...
for i in range(2000):
    ...
    zc = 10**rng.uniform(-3.5, 1.3)
    s = zc*8*r*t
    if s > 0:
        d = abs(ref(r, s, t) - pbc_c(r, s, t))
        if d > worst: worst, wpt = d, (r, s, t)
print("worst abs diff:", worst, "at", wpt)
